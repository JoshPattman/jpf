...
func (r *rateLimitedModel) Respond(ctx context.Context, msgs []jpf.Message, opts ...jpf.ModelResponseOpt) (jpf.ModelResponse, error) {
	err := r.limiter.Wait(ctx)
	if err != nil {
		return jpf.ModelResponse{}, errors.Join(errors.New("failed to wait for rate limiter"), err)
	}
	return r.model.Respond(ctx, msgs, opts...)
}
