...
func NewGeminiModel(key, modelName string, opts ...GeminiModelOpt) jpf.Model {
	model := &geminiModel{
		key:          key,
		model:        modelName,
...
func (c *geminiModel) Respond(ctx context.Context, msgs []jpf.Message) (jpf.ModelResponse, error) {
	failedUsage := jpf.Usage{FailedCalls: 1}
	failedResp := jpf.ModelResponse{Usage: failedUsage}

	body, err := c.createBodyData(msgs)
	if err != nil {
		return failedResp, utils.Wrap(err, "could not create request body")
	}
	...
	if err != nil {
		return jpf.ModelResponse{Usage: usage.Add(jpf.Usage{FailedCalls: 1})},
			utils.Wrap(err, "failed to parse response: %s", string(rawRespBytes))
	}
...
func (c *geminiModel) createBodyData(msgs []jpf.Message) (io.Reader, error) {
...
func roleToGemini(role jpf.Role) (string, error) {
	switch role {
	case jpf.UserRole:
		return "user", nil
...
func (m *geminiModel) messagesToGemini(msgs []jpf.Message) (string, []map[string]any, error) {
