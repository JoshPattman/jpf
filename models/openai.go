...
func NewOpenAIModel(key, modelName string, opts ...OpenAIModelOpt) jpf.Model {
	model := &openAIModel{
		key:             key,
		model:           modelName,
		maxOutput:       0,
...
type openAIModel struct {
	...
	reasoningRole      jpf.Role
	reasoningTransform func(string) string
	streamCallbacks    *streamCallbacks
}

func (c *openAIModel) Respond(ctx context.Context, msgs []jpf.Message) (jpf.ModelResponse, error) {
	failedUsage := jpf.Usage{FailedCalls: 1}
	failedResp := jpf.ModelResponse{Usage: failedUsage}
	body, err := c.createBodyData(msgs)
	if err != nil {
		return failedResp, utils.Wrap(err, "could not encode body")
	}
	req, err := c.createRequest(ctx, body)
	if err != nil {
		return failedResp, utils.Wrap(err, "could not create request")
	}
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return failedResp, utils.Wrap(err, "could not execute request")
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if err := json.Unmarshal(respData, &errResp); err != nil {
			return failedResp, utils.Wrap(fmt.Errorf("http status %d", resp.StatusCode), "request failed: %s", string(respData))
		}
	...
	if err != nil {
		return jpf.ModelResponse{Usage: usage.Add(jpf.Usage{FailedCalls: 1})}, utils.Wrap(err, "failed to parse response: %s", string(rawRespBytes))
	}
	if respTyped.Error.Code != "" {
		return jpf.ModelResponse{Usage: usage.Add(jpf.Usage{FailedCalls: 1})}, &openAIError{
	...
	if len(respTyped.Choices) == 0 {
		return jpf.ModelResponse{Usage: usage.Add(jpf.Usage{FailedCalls: 1})}, utils.Wrap(err, "response had no choices: %s", string(rawRespBytes))
	}
...
func (c *openAIModel) createBodyData(msgs []jpf.Message) (io.Reader, error) {
...
func roleToOpenAI(role jpf.Role) (string, error) {
	switch role {
	case jpf.SystemRole:
		return "system", nil
	case jpf.UserRole:
...
func (m *openAIModel) messagesToOpenAI(msgs []jpf.Message) (any, error) {
	jsonMessages := make([]map[string]any, 0)
	for _, msg := range msgs {
		role := msg.Role
		contentStr := msg.Content
		if role == jpf.ReasoningRole {
