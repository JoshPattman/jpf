...
func (f *filePersistCache) GetCachedResponse(ctx context.Context, salt string, msgs []Message) (bool, []Message, Message, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *filePersistCache) SetCachedResponse(ctx context.Context, salt string, inputs []Message, aux []Message, out Message) error {
	f.mu.Lock()
